...
function jobLoc(job){
  ...
  const zip = String((job && job.zip) || '').trim();
  if(/^\d{5}(-\d{4})?$/.test(zip)) return city ? `${city} ${esc(zip)}` : esc(zip); // real postal ZIP
  const m = zip.match(/,\s*([A-Za-z]{2})\s*$/);                                     // synthetic "PFX:City,ST"
  return m ? (city ? `${city}, ${esc(m[1].toUpperCase())}` : esc(m[1].toUpperCase())) : city;
}
...
const SKILL_FOR = { cna:'cna', patient_care_tech:'cna', home_health_aide:'cna', caregiver:'cna', med_tech:'cna',
  ...
  radiology_tech:'radiology_tech', med_lab_tech:'medical_assistant', dental_assistant:'medical_assistant',
  ...
  tool_die:'tool_die', injection_molding:'injection_molding', press_operator:'machine_operator', material_handler:'machine_operator', robotics_tech:'robotics_tech',
  ...
  // each fab module now has its own dedicated discipline-specific skill check
  photolith_tech:'photolith_tech', etch_tech:'etch_tech', deposition_tech:'deposition_tech', cmp_tech:'cmp_tech', implant_tech:'implant_tech', diffusion_tech:'deposition_tech', packaging_tech:'cleanroom_op',
  equipment_tech:'equipment_tech', equipment_engineer:'equipment_tech', field_service:'equipment_tech', fab_facilities:'equipment_tech', cal_tech:'metrology_tech',
